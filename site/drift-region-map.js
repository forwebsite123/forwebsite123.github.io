(function(){
  ...
  async function init(card){
    ...
    const map=L.map(el,{zoomControl:false,attributionControl:false,scrollWheelZoom:true,touchZoom:true,dragging:true,doubleClickZoom:false,boxZoom:false,keyboard:false,zoomSnap:.05,zoomDelta:.9,wheelPxPerZoomLevel:24,wheelDebounceTime:18,maxBounds:safeBoundsFor(region),maxBoundsViscosity:1,worldCopyJump:false});
    ...
    map.fitBounds(safeBoundsFor(region),{animate:false,padding:[12,12]}); map.setMinZoom(map.getBoundsZoom(safeBoundsFor(region),false)); const maxZoomDelta=REGION_MAX_ZOOM_DELTA[region] || 4.0; map.setMaxZoom(map.getMinZoom()+maxZoomDelta); map.setZoom(map.getMinZoom(),{animate:false}); map.scrollWheelZoom.enable(); clampMap(map, region);
    ...
    const buttonZoom=delta=>{ const nextZoom=clamp(map.getZoom()+delta,map.getMinZoom(),map.getMaxZoom()); map.setZoomAround(map.getCenter(),nextZoom,{animate:!matchMedia('(prefers-reduced-motion: reduce)').matches}); setTimeout(()=>clampMap(map, region), 120); };
    card.querySelector('[data-region-map-zoom="in"]')?.addEventListener('click',()=>buttonZoom(1.05));
    card.querySelector('[data-region-map-zoom="out"]')?.addEventListener('click',()=>buttonZoom(-1.05));
